...
>
> 发布通道：`v0.1.0-preview.1` 已通过验收（非正式预览）
>
...
绨络（Tylpet）是一套轻量、可扩展的桌面伙伴程序；小洛宝是当前第一位内置角色。她安静地待在桌面角落，通过自然的小动作提供陪伴，未来可以一起专注、聊天，并在你用其他应用播放音乐时轻轻跟随律动；在用户授权后，她也能对当前工作状态作出克制的反馈。她不是普通工具悬浮窗，而是有持续性格和关系感的桌面伙伴。
...
- Windows 原生右键菜单：切换始终置顶、回到当前显示器中央、打开对话、退出绨络。
...
- 根 README、变更日志和 M16-P0 发布验收矩阵已建立；`v0.1.0-preview.1` NSIS 安装包、自动化、安装/覆盖/卸载与品牌升级复验均已通过。

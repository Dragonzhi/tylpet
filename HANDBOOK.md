...
- 对话可选开启受控 Agent：模型只获得 v1 白名单语义工具，可请求命名动作、表情、视线、语义窗口移动和番茄钟操作；所有副作用都在主窗口重新校验并经过统一调度器，窗口移动与取消计时需要逐次确认。
...
- 模型通信通过 Provider 接口隔离；纯文本对话不附带工具，只有用户开启 Agent 后才会暴露经过白名单和调度器双重校验的语义工具。

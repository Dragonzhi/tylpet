...
绨络（英文名：Tylpet）是一个运行在 Windows 桌面上的轻量、可扩展桌面伙伴；“小洛宝”是当前内置角色。当前 `0.1.0-preview.1` 是开发预览版本：基础桌宠完全离线可用，模型对话、Agent、语音、系统音乐反应、长期记忆和创作者插件均为可选能力。
...
1. 从对应预览 tag 的构建产物取得 `绨络_*_x64-setup.exe`。
...
根目录的 [`VERSION`](./VERSION) 是主程序唯一版本源。发布新版本时不要分别手改 npm、Cargo 和 Tauri 版本：

```powershell
# 1. 只修改 VERSION，例如 0.1.0-preview.2
npm run version:sync
npm run release:verify
npm run tauri build

# 2. 更新 CHANGELOG 日期并提交全部发布改动后
npm run release:tag
git push origin v0.1.0-preview.2
```

`version:check` 会校验 `package.json`、`package-lock.json`、`Cargo.toml`、`Cargo.lock` 和 `tauri.conf.json`。`release:tag` 根据 `VERSION` 创建 `v<version>` 注释标签，但不会自动 push，也拒绝在工作区不干净或 CHANGELOG 尚未填写发布日期时运行。

Animation Studio 仍有独立版本，不由根 `VERSION` 同步。历史应用标识符 `com.tauri-app.ltypet`、插件清单名 `ltypet.plugin.json` 和内部包命名空间 `@ltypet/*` 暂时保留为 v1 兼容标识；它们不代表当前产品英文名。
...
仓库目前没有授予覆盖全部源码与素材的统一开源许可证；除各第三方依赖自身许可证外，默认保留所有权利。“绨络 / Tylpet”是项目名称，“小洛宝”是当前角色昵称；角色设计、名称与衍生素材不因源码可见而自动获得复制、再发行或商业使用许可。
